...
%type  <ptr> 
        program ExtDefList ExtDef StructSpecifier OptTag Tag Specifier ExtDecList FuncDec
...
%token  IF ELSE WHILE RETURN INC DEC INCASS DECASS
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//赋值表达式；$$结点type_id空置未用，正好存放运算符
      ...
      | INT           {$$=mknode(SINT,NULL,NULL,NULL,yylineno);$$->type_int=$1;$$->type=INT;}
      | FLOAT         {$$=mknode(SFLOAT,NULL,NULL,NULL,yylineno);$$->type_float=$1;$$->type=FLOAT;}
      | CHAR          {$$=mknode(SCHAR,NULL,NULL,NULL,yylineno);$$->type_char=$1;$$->type=CHAR;}
      ;
